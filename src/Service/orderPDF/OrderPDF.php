...
class OrderPDF extends Fpdi
{
    ...
    function OrderHeader(Order $order, array $aInfo, $info = null)
    {
    ...
    function CareListHeader(Order $order)
    {
        $this->CarePage = 1;
    ...
    function CareListBody(Product $Product, Order $order)
    {
        if ($this->getY() > 200) {
            $this->addPage();
            $this->CarePage++;
    ...
    function ItemsBody($item, $cellHight)
    {
        ...
        $this->SetFont('Helvetica', '', 12);
        $this->Cell(20, $cellHight, sprintf("%01.2f ", $item['price']) . ' Eur', '', 1, 'L');
    ...
    function PicklistHeader($Order, $Picklistname)
    {
        ...
        $this->SetFont('Helvetica', '', 10);
        $this->SetTextColor(0, 0, 0);
        $this->Cell(25, 8, "für die Aufträge:", '', 1, 'L');
        foreach ($Order as $o) {
            $this->SetFont('Helvetica', '', 10);
    ...
    function ItemsPickBody($item, $cellHight)
    {
        ...
        $this->Cell(20, $cellHight, sprintf("%01.2f ", $item['item']->getPrice()) . ' Eur', 'T', 0, 'L');
        $STROrder = '';
