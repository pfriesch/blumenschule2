...
class OrderController extends AbstractController
{
    public function indexAction()
    {
        ...
        $dql = "SELECT a FROM BSApp:Orders a";
    ...
    public function exportedAction()
    {
        ...
        // $oPlentyMarketsAPI	=	new PlentyMarketsAPI($this,$this->getDoctrine());
        // $back =  7;
        // $date = date('U',mktime(0, 0, 0, date("m")  , date("d") - $back , date("Y"))) ;
        // $oPlentyMarketsAPI->doGetOrdersWithState( 7, $date  );
...
//        TODO paginate with rest api maybe?
    ...
    public function printAction(Request $request, OrderPDFService $pdfService)
    {
        ...
        if (!file_exists($printTempDir . "print/" . $PickListName . ".pdf")) {
            ...
            $aOrders = array();
            //$orders = $oPlentyMarketsAPI->doGetOrdersWithState(array('OrderStatus'=> doubleval(5)));
...
//        $oPlentyMarketsAPI	=	new PlentyMarketsAPI($this,$this->getDoctrine());
//            $request = $oPlentyMarketsAPI->doGetItemsBaseByOptions(array('ItemID'=>$ArtileID[0]));
//           if(count($request) > 0){
//               $product = $request[0];
//           }else{
